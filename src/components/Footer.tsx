      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          <div className="space-y-6">
            <h3 className="text-2xl font-bold">{settings?.siteName || 'ApnarPonno'}</h3>
            ...
            <div className="flex items-center gap-4">
              {settings?.facebookUrl && (
                <a href={settings.facebookUrl} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center hover:bg-secondary transition-all">
                  <Facebook className="h-5 w-5" />
                </a>
              )}
              {settings?.instagramUrl && (
                <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center hover:bg-secondary transition-all">
                  <Instagram className="h-5 w-5" />
                </a>
              )}
              {settings?.youtubeUrl && (
                <a href={settings.youtubeUrl} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center hover:bg-secondary transition-all">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-white/70">
              <li className="flex items-start gap-3">
                ...
                <span>{settings?.address || 'House 12, Road 5, Sector 3, Uttara, Dhaka'}</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="h-5 w-5 text-secondary shrink-0" />
                <span>{settings?.contactPhone || '+880 1234 567890'}</span>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="h-5 w-5 text-secondary shrink-0" />
                <span>{settings?.contactEmail || 'support@apnarponno.com'}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-white/10 text-center text-xs text-white/40">
          <p>© 2026 {settings?.siteName || 'ApnarPonno'}. All Rights Reserved. Pure & Natural.</p>
        </div>
      </div>
    </footer>
  );
